...
void CMFCHW2Dlg::OnBnClickedButtonCreatedata()
{
	...
	int end_number = 100;
		...
		for(int i = 0; i < 10; i++)
		{
			...
			str[i + 1].Format(_T("%d"), random_number);
...
void CMFCHW2Dlg::OnBnClickedButtonSort()
{
	...
	for(int i = 0; i < 10; i++)
	{
		compare_data[i+1] = _ttoi(str[i + 1]);
	}
	...
	for(i = 1; i < 11; i++)
	{
		for(j = 1; j < 11 - i; j++)
		{
			if(compare_data[j] > compare_data[j+1])
			{
				temp = compare_data[j];
				compare_data[j] = compare_data[j+1];
				compare_data[j+1] = temp;                                    
			}
		}
		if ( i < 10)
		{		
	...
	for(int i = 0; i < 10; i++)
	{
		str[i + 1].Format(_T("%d"), compare_data[i+1]);
	}
	m_Edit1 = str[1];
	m_Edit2 = str[2];
	m_Edit3 = str[3];
